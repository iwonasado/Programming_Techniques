...
// recruit constructor receiving position in map and the unit name
recruit::recruit(size_t team_index, bool hidden, const std::string& unit_name, const map_location& recruit_hex):
		...
		temp_unit_(create_corresponding_unit()), // auto-pointer ownership transfer
...
recruit::recruit(config const& cfg, bool hidden)
	...
	, cost_(0)
{
	// checks wether unit name is valid
...
// initiate the recruitment setting initial location, movement and other actions
void recruit::init()
{
...
unit_ptr recruit::create_corresponding_unit()
{
	...
	return result; //ownership gets transferred to returned auto_pointer copy
}

// checks if the recruitment is valid
