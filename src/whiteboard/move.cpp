...
namespace wb {
	...
	move::move(size_t team_index, 
			   bool hidden,
			   unit& u, 
			   const pathfind::marked_route& route,
			   arrow_ptr arrow, 
			   fake_unit_ptr fake_unit): action(team_index,hidden),
	...
	void move::init()
	{
		...
		{	
			if(arrow_style == arrow::STYLE_HIGHLIGHTED)
			{
			...
			{
				if(arrow_style == arrow::STYLE_FOCUS)
				{
				...
				{ 	
					if(arrow_style == arrow::STYLE_FOCUS_INVALID)
					{
						arrow_brightness_ = ARROW_BRIGHTNESS_STANDARD;
	...
	void move::execute(bool& success, bool& complete)
	{
		...
		{
			 if ( unit_it == resources::units->end()  ||  unit_it->id() != unit_id_ )
    		 {
