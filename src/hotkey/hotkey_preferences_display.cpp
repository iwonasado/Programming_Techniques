...
namespace preferences {
	...
	class hotkey_preferences_parent_dialog: public gui::dialog {
		...
		public:
			...
			hotkey_preferences_parent_dialog(display &disp, 
											 ...
											 clear_buttons_(false), hotkey_cfg_(),
											 resetter_(disp, hotkey_preferences_dialog) {

				gui::dialog_button* reset_button = new gui::dialog_button(disp.video(), _("Defaults"), 
	...
	hotkey_preferences_dialog::hotkey_preferences_dialog(display& disp) : gui::preview_pane(disp.video()),
														 add_button_(disp.video(), _("Add Hotkey")),
														 clear_button_(disp.video(),	_("Clear Hotkey")),
											//SCOPE_COUNT means "hotkey with more than one scope" in this case
														 tab_(hotkey::SCOPE_COUNT), 
														 general_commands_(), game_commands_(), 
														 editor_commands_(), title_screen_commands_(),
														 heading_( (formatter() << HEADING_PREFIX 
														 << COLUMN_SEPARATOR << _("Action")
														 << COLUMN_SEPARATOR << _("Binding")).str() ),
														 selected_command_(0), general_sorter_(), game_sorter_(),
														 editor_sorter_(), title_screen_sorter_(),
											// Note: If you don't instantiate the menus with heading_,
											// the header can't be enabled later, seems to be a bug in gui::menu
														general_hotkeys_(disp.video(), 
														boost::assign::list_of(heading_), 
														false, -1, -1, &general_sorter_, 
														&gui::menu::bluebg_style), game_hotkeys_(disp.video(), 
														boost::assign::list_of(heading_), false, -1, -1, 
														&game_sorter_, &gui::menu::bluebg_style),
														editor_hotkeys_(disp.video(), 
														boost::assign::list_of(heading_), false, -1, -1, 
														&editor_sorter_, &gui::menu::bluebg_style), 
														title_screen_hotkeys_(disp.video(), 
														boost::assign::list_of(heading_), false, -1, -1, 
														&title_screen_sorter_, &gui::menu::bluebg_style), 
														disp_(disp), parent(NULL) {
	...
	void hotkey_preferences_dialog::set_hotkey_menu(bool keep_viewport) {
		...
		symbols["hotkey_description"] = hotkey::get_description((*commands)[selected_command_]);

		const std::string clear_text = vgettext(hotkey_preferences_dialog::clear_button_text, symbols);
		clear_button_.set_help_string(clear_text);
		const std::string   add_text = vgettext(hotkey_preferences_dialog::add_button_text, symbols);
			...
			if (oldhk && oldhk->active()) 
			{
				if (oldhk->get_command() != id) 
				{
					...
					std::string text = vgettext("\"$hotkey_sequence|\" is in use by \
												 \"$old_hotkey_action|\". Do you wish to reassign it to \
												 \"$new_hotkey_action|\"?", symbols);

					const int res = gui2::show_message(disp_.video(), _("Reassign Hotkey"), text,
