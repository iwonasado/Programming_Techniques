...
namespace {
	...
	hotkey::hotkey_item& get_hotkey(int mouse, int joystick, int button, int hat, int value,
		...
		for (itor = hotkeys_.begin(); itor != hotkeys_.end(); ++itor) 
		{
			...
			if ( !( hotkey::is_scope_active(hotkey::get_hotkey_command(itor->get_command()).scope) && 
				    itor->active() ) ) 
			{
				continue;
			}
...
namespace hotkey {
	...
	const hotkey_item& get_hotkey(int mouse, int joystick, int button, int hat, int value,
		...
		return ::get_hotkey(mouse, joystick, button, hat, value, shift, ctrl, cmd, alt);
	}

	const hotkey::hotkey_item& get_hotkey(int character, int keycode,
										  bool shift, bool ctrl,	bool cmd, bool alt) {

		return ::get_hotkey(character, keycode, shift, ctrl, cmd, alt);
	}
	...
	void add_hotkey(const hotkey_item& item) {
		...
		hotkey_item& old_hk = (item.get_mouse() != -1 || item.get_joystick() != -1) ?
							   ::get_hotkey(item.get_mouse(), item.get_joystick(), item.get_button(), 
							   item.get_hat(), item.get_value(), item.get_shift(), item.get_ctrl(), 
							   item.get_cmd(), item.get_alt()) : ::get_hotkey(item.get_character(), 
							   item.get_keycode(), item.get_shift(), item.get_ctrl(), item.get_cmd(), 
							   item.get_alt());
	...
	std::string hotkey_item::get_name() const {
		...
		if (mouse_     >=  0) 
		{ 
			str << _("Mouse") << mouse_ << _("Button") << button_; 
		}
		else
		{
			// EMPTY
		...
		if (joystick_  >=  0) 
		{ 
			str << _("Joystick") << joystick_ << _("Button") << button_; 
		}
		else
		{
	...
	void hotkey_item::save(config& item) const {
		...
		if (get_keycode()   >= 0) 
		{
			item["key"]      = SDL_GetKeyName(SDLKey(get_keycode()));
		}
		else
		{
			// EMPTY
		...
		if (get_character() >= 0) 
		{
			item["key"]      = unicode_cast<utf8::string>(static_cast<ucs4::char_t>(get_character()));
		}
		else
		{
			// EMPTY
